        ...
        if FORCE_SUB_CHANNELS:
            try:
                ...
                for x in FORCE_SUB_CHANNELS:
                    links.append((await self.get_chat(x)).invite_link)
                if len(links) == 0:
                    for x in FORCE_SUB_CHANNELS:
                        await self.export_chat_invite_link(x)
                        links.append((await self.get_chat(x)).invite_link)
                ...
                self.LOGGER(__name__).warning(
                    f"Please Double check the FORCE_SUB_CHANNELS value and Make sure Bot is Admin in channelS with Invite Users via Link Permission, Current Force Sub Channels Value: {' '.join(FORCE_SUB_CHANNELS)}")
